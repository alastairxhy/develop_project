layui.use(["layer","form","element"],function(){
	...
	var tbSeEvalId = getQueryVariable("tbSeEvalId");
		...
		beforeCreate:function(){
			...
			$.ajax({
					...
					data:{
						...
						corpId:'4',
						tbSeEvalId:tbSeEvalId,
                        tbSeEvalClassifyId:getCookie("tbSeEvalClassifyId")
					},
					dataType:"json",
			...
			form.on("submit(risk_garde_submit)",function(data){
				...
				$.ajax({
					...
					success:function(res){
						if(res.success){
							layer.closeAll("loading");
							window.location.replace("regulator_rule.html");
